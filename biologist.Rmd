...
#select 1k diff expressed genes
#subset it from fpkm tables
...
rownames(concat04) <- NULL
concat_fin <- concat04[unique(concat04$gene_short_name), ] %>% column_to_rownames("gene_short_name")
...
#P01_fpkm$status <- rep("P0_1",nrow(P01_fpkm))
#P01_fpkm$samples <- paste(P01_fpkm$status, P01_fpkm$gene_short_name)

#P01_final <- P01_fpkm %>% column_to_rownames(sample)


P02_fpkm <- P0_2 %>% select(gene_short_name,FPKM) 
P41_fpkm <- P4_1 %>% select(gene_short_name,FPKM) 
P42_fpkm <- P4_2 %>% select(gene_short_name,FPKM)
P71_fpkm <- P7_1 %>% select(gene_short_name,FPKM)
P72_fpkm <- P7_2 %>% select(gene_short_name,FPKM)
ad1_fpkm <- Ad1_ftable %>% select(gene_short_name, FPKM)
ad2_fpkm <- Ad2_ftable %>% select(gene_short_name, FPKM)



#concat <- inner_join(P01_fpkm, P02_fpkm, by="gene_short_name") %>% inner_join(., P41_fpkm, by="gene_short_name") %>% inner_join(., P42_fpkm, by="gene_short_name") %>% inner_join(., P71_fpkm, by="gene_short_name") %>% inner_join(., P72_fpkm, by="gene_short_name") %>% inner_join(., ad1_fpkm, by="gene_short_name") %>% inner_join(., ad2_fpkm, by="gene_short_name")


#how to make samples columns 
#concatenate things? 
